...
# ---------------------------------------------------------------------------- #
# [internal only-prep & clean dataset]
...
# Format data types
grab <- transform(grab, DatetimeSG = as.Date(DatetimeSG), 
...
uber <- transform(uber, DatetimeSG = as.Date(DatetimeSG), 
                  post_username = as.character(post_username),
                  post_ID = as.factor(post_ID), 
                  post_comment = as.character(post_comment),
                  message_ID = as.factor(message_ID),
...
# From there, select only data from March to Aug
...
# ---------------------------------------------------------------------------- #
# [plot 1 = num posts per day per company as TIMESERIES line graph]

# Group by number of posts per company
grab1 <- grab %>% 
...
# ---------------------------------------------------------------------------- #
# [plot 3 = num posts per month, back-to-back bar chart]

# Format data for MONTH clusters
timeseries3 <- mutate(timeseries2, Month=month(DatetimeSG)) %>%
...
# ---------------------------------------------------------------------------- #
# [Engineer features for word count, tidy up stuff]
